    ...
    public partial class AddAggregations : Migration
    {
        ...
        private void AddMaterializedView(MigrationBuilder migrationBuilder, string name, string timeBucket)
        {
            migrationBuilder.Sql(
                $"CREATE MATERIALIZED VIEW IF NOT EXISTS {name} (time, avgValue, diffValue, maxValue, minValue, countValue, nodeInstanceId) " +
            ...
            migrationBuilder.Sql($"SELECT remove_continuous_aggregate_policy('{name}'," +
                                 $"if_not_exists => true);", true);
