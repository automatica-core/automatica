...
namespace P3.Driver.HomeKit.BonjourOld
{
    public class MdnsService
    {
        ...
        private int _announcementsSent = 0;
        ...
        private void TimerOnElapsed(object se, ElapsedEventArgs ev)
        {
            ...
            lock (_timer)
            {
                try
                {
                    ...
                    if (_announcementsSent >= 3)
                    {
                        dataToSend = GenerateAnnouncement(_socket.AddressFamily);
                    }
                    else
                    {
                        dataToSend = GenerateDnsRecord(_socket.AddressFamily);
                    }
        ...
        {
            _socket.Connect(new IPEndPoint(MulticastAddressIp4, MulticastPort));
            ...
            while (_isRunning)
            {
                try
                {
                    ...
                    {
                        if (GetIPAddresses().Any(a => a.ToString() == senderRemoteIp.Address.ToString()))
                        {
                            return;
                        }
