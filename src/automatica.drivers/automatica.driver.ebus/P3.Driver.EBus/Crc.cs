using System;

namespace P3.Driver.EBus
{
    internal static class Crc
    {
        public static byte Crc8(ReadOnlySpan<byte> data)
        {
            byte crc = 0;

            foreach (var b in data)
            {
                crc = Crc8(crc, b);
            }

            return crc;
        }

        public static byte Crc8(byte crc, byte data)
        {
            return (byte)(_crc8table[crc] ^ data);
        }

        private static byte[] _crc8table = new byte[] {,  };
    }
}
